...
> **_NOTE_**  you'll need a schema in your database, so to test in local the _prod_ profile (using the above postgres container), set _-Dfruit.schema.create=true_ as follows: _java -Dfruit.schema.create=true -jar target/quarkus-app/quarkus-run.jar_.
